...
def main(now: datetime | None = None):
    ...
    logging.info("-------------- START SCRIPT: dynamic_load.pitch ------------")
    ...
    for i, row in df_wf_config.iterrows():
        ...
        output_table_name = f"realtime_pitch_{wf_name}"
        ...
        if check:
            ...
            df_last_times: pd.DataFrame = run_query(
                f"max_time_pitch_{wf_name}", config["db"], returns="Dataframe"
            )
