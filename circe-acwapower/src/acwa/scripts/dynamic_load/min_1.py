"""
acwa.scripts.dynamic_load.min_1

Complete process to write the dynamic raw input of 1 min data
"""
...
def main(now: datetime | None = None):
    ...
    logging.info("----------- START SCRIPT: dynamic_load.min_1 ---------------")
    ...
    for i, row in df_wf_config.iterrows():
        ...
        write_dynamic_raw_table(
            f"input_1min_{wf_name}", # Input data (collection of input queries to use)
            f"realtime_input_1min_{wf_name}", # Output table to write
            config = config,
