"""
acwa.scripts.collection.min_10

Script to collect the raw data (10 min) of the wind farms, into a general table
"""
...
def main(incremental=True):
    ...
    logging.info("------------ START SCRIPT: collection.min_10 ---------------")

    output_table_name = "input_10min"
    output_schema = "intermediate"

    logging.info("Load the mapping information")
    df_map, df_var = data.load_mapping_information(config['db'], "10min")
    ...
    for i, row in df_wf.iterrows():
        ...
        logging.info(f"Extracting 10-min signals for Wind Farm {wf_name}")

        df_map_wf = df_map[df_map["id"] == id_wf]

        logging.info("Check incremental flag")
        replace_flag = data.obtain_replace_flag(
            output_table_name, config["db"], output_schema, incremental, id_wf, i
        )

        logging.info("Extract time horizon")
        max_datetime = (
            data.extract_maximum_datetime(config["db"], id_wf)
            if not replace_flag
            else None
        )
        ...
        for table in lst_tables:
            ...
            df_data = data.DICT_MAP_10MIN[table](
